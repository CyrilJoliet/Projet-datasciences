...
def log_to_file(file_path, message):
    """
    Appends a timestamped message to a text file and automatically closes it.
    """
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # Format: YYYY-MM-DD HH:MM:SS
    with open(file_path, 'a') as file:  # Open file in append mode
        file.write(f"[{timestamp}] : {message}\n")  # Append timestamp and message
file_path = "C:/Users/abdel/Desktop/log.txt"  # Path to the text file
            ...
            #change color and ad regressions on visualisation
                ...
                if  -15 <= (Left[2] <= 0 and 0 <= Right[2]<= 15) and (Left[3]>0.8 and Right[3]>0.8) and (0>variation_L or 0>variation_R):
                    ...
                    if orange_condition_counter >5 and T>0:
                        ...
                        log_to_file(file_path, f"Alert red à {end_time.time()}")  # Write log entry
...
log_to_file(file_path, "=============================================="+f"execution of file {file_name} is done"+"\n")     
